...
void read_file(char FN1[],char FN2[],char FN3[],int nx,int nz,int nnx,int nnz,float dx,float dz,float favg,float dt,
               float *v,float *e,float *d,int npml,int nzv)
{
		 int i,j,id;
		 ...
		 {
                      fseek(fp1,(i-npml)*nzv*sizeof(float),0);
                      fseek(fp2,(i-npml)*nzv*sizeof(float),0);
                      fseek(fp3,(i-npml)*nzv*sizeof(float),0);
			 for(j=npml;j<nz+npml;j++)
			 {
                            id=i*nnz+j;
				 fread(&v[id],4L,1,fp1);v[id]*=0.95;
				 fread(&e[id],4L,1,fp2);e[id]*=0.9;
				 fread(&d[id],4L,1,fp3);d[id]*=0.9;
...
{
	int is, it, nx, nz, nzv, nnx, nnz, nt, wtype, na, da, dcdp;
...
/********** dat document ***********/
          ...
          char FN1[250]={"layer_vel_1001_301.dat"};
          char FN2[250]={"layer_epsilon_1001_301.dat"};
          char FN3[250]={"layer_delta_1001_301.dat"};
	  ...
	  char FN10[250]={"layer_stkadcigs.dat"};
	  char FN11[250]={"layer_smiled_adcigs.dat"};
...
/************a*************/
        ...
        read_file(FN1,FN2,FN3,nx,nz,nnx,nnz,dx,dz,favg,dt,v,e,d,npml,nzv);
/****************************/
